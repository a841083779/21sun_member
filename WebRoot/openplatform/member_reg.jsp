...
<%
		//Map sinaUserInfoSESSION = (HashMap)session.getAttribute("sinaUserInfoSESSION") ;  // 新浪微博帐号信息
		String flag = Common.getFormatStr(request.getParameter("flag")) ;
		if("sina".equals(flag)){
			session.removeAttribute("qqUserInfoSESSION") ;
		}
		if("qq".equals(flag)){
			session.removeAttribute("sinaUserInfoSESSION") ;
		}
		User sianUserInfo = (User)session.getAttribute("sinaUserInfoSESSION") ;  // 新浪微博帐号信息
		UserInfoBean qqUserInfoSESSION = (UserInfoBean)session.getAttribute("qqUserInfoSESSION") ; 
		if(null==sianUserInfo && null==qqUserInfoSESSION){
...
<style type="text/css">
.registBg h2 { font-family: "微软雅黑"; font-size: 20px; height: 29px; }
.registBg img.show { float: left; height: 50px; margin-right: 10px; width: 50px; }
.registBg p { font-family: "微软雅黑"; font-size: 16px; font-weight: bold; margin: 0; padding: 0; }
.registBg p a { font-size: 14px; font-weight: normal; margin-left: 5px; text-decoration: underline; }
</style>
...
<div class="registForm">
  <div class="registBg">
    <div class="registTop" style="height:64px;width:80%;margin:30px 75px;"> <img class="show" src="<%=(sianUserInfo==null || flag.equals("qq"))?(qqUserInfoSESSION==null?"/":Common.getFormatStr(session.getAttribute("figureurl50"))):Common.getFormatStr(sianUserInfo.getProfileImageUrl()) %>" onerror="showImgDelay(this,'/images/nopic.gif',2);"/>
      <h2>HI!<%= (sianUserInfo==null || flag.equals("qq"))?(qqUserInfoSESSION==null?"":Common.getFormatStr(qqUserInfoSESSION.getNickname())):Common.getFormatStr(sianUserInfo.getScreenName()) %></h2>
      <p> 恭喜您注册成功，为了您的账户安全请完善以下资料； <a href="javascript:;" class="bd" onclick="changeOpt('bd')">已有商贸网帐号，绑定登录</a> <a href="javascript:;" style="display:none;" class="fh" onclick="changeOpt('fh')">返回完善资料</a> </p>
    </div>
    <div class="registTable">
      <form id="theform" name="theform" method="post" action="/openplatform/member_reg_action.jsp"  onsubmit="return regYanzheng();" >
        <input type="hidden" id="zd_province" name="zd_province"  />
        <input type="hidden" id="zd_city" name="zd_city"  />
        <table width="98%" border="0" cellpadding="0" cellspacing="0" class="bd">
          <tr>
            <th width="37%"><font>*</font> 用户名：</th>
            <td width="63%"><input type="text" name="mem_no" id="mem_no" class="ri" onblur="formyanzheng('mem_no')" value="<%=(sianUserInfo==null || flag.equals("qq"))?(qqUserInfoSESSION==null?"":Common.getFormatStr(qqUserInfoSESSION.getNickname())):Common.getFormatStr(sianUserInfo.getScreenName()) %>"/>
              ...
              <input type="hidden" name="sina_id" id="sina_id" value="<%= sianUserInfo==null?"":Common.getFormatStr(sianUserInfo.getId())%>"/>
              
              <!-- qq 帐号 id -->
              
              <input type="hidden" name="qq_id" id="qq_id" value="<%=Common.getFormatStr(qqUserInfoSESSION==null?"":session.getAttribute("qqOpenId")) %>" /></td>
          </tr>
          <tr>
            <th><font>*</font> 密码：</th>
            <td colspan="2" ><div style="clear:both">
                <input type="password" name="passw" id="passw" class="ri" onfocus="showText('passw')" onblur="formyanzheng('passw')" onkeyup="pwStrength(this.value)"/>
              ...
              <div style="width:100%; float:left;">
                ...
                <div style="float:left;"><span style="float:left; padding:3px 3px 0px 0px;">
                  <input type="checkbox" name="showPwdFlag" id="showPwdFlag" onclick="showPwd();"/>
                  </span> <span style="float:left; *line-height:26px;">显示密码字符</span></div>
              </div></td>
          </tr>
          <tr>
        ...
        <!-- ------------------------------------------已有商贸网帐号------------------------------------------------------- -->
        <table width="98%" border="0" cellpadding="0" cellspacing="0" class="fh" style="display:none;">
          <tr>
            <th width="37%"><font>*</font> 用户名：</th>
            <td width="63%"><input type="text" name="mem_no" id="mem_no" class="ri" onblur="bidingCount('mem_no')"/>
              <div class="diu" id="mem_no_dui" style="display:none"></div>
              <div class="cuo" id="mem_no_cuo" style="display:none"></div>
              <div class="cuo1" id="mem_no_cuo_info" style="display:none"></div>
              <div id="mem_no_show_info" style="display:none; clear:both;">4-18个字母、数字、"-"或"_"，不支持中文和特殊符号。</div>
          ...
          <tr>
            <th><font>*</font> 密码：</th>
            <td colspan="2" ><div style="clear:both">
                <input type="password" name="passw" id="passw" class="ri" onfocus="showText('passw')" onblur="formyanzheng('passw')"/>
                <input type="text" id="passw_show" class="regi" onblur="formyanzheng('passw')" onkeyup="pwStrength(this.value)" style="display:none"/>
                <div class="diu" id="passw_dui" style="display:none"></div>
                <div class="cuo" id="passw_cuo" style="display:none"></div>
                <div class="cuo1" id="passw_cuo_info" style="display:none"></div>
          ...
          <tr>
            <th height="30">&nbsp;</th>
            <td><input type="button" onclick="mySubmit();" name="regid" id="regid" value="同意以下协议，绑定" class="registBtn" /></td>
          </tr>
        </table>
        <div class="xieyi">
          <div>
...
<script type="text/javascript">
	...
	var tableobjBd = '' ;
	var tableobjFh = '' ;
	function changeOpt(tag){
		if('bd'==tag){
		tableobjBd = jQuery("table.bd").detach() ;
		if(''!=tableobjFh){
			 jQuery(tableobjFh).insertBefore(jQuery("#theform div:eq(0)")) ;
		}
		  jQuery('table.fh , .registTop a.fh').show();
	  	 jQuery('.registTop a.bd').hide();
		}
		if('fh'==tag){
			 tableobjFh = jQuery("table.fh").detach() ;
			jQuery(tableobjBd).insertBefore(jQuery("#theform div:eq(0)")) ;
		   jQuery('.registTop a.bd').show();jQuery('.registTop a.fh').hide()
		}
	}
	// 绑定帐户
	function bidingCount(obj){
		var inputValue = jQuery("#"+obj).val() ; // 用户输入的值
		jQuery.ajax({
			type:'post',
			url:'/tools/ajax.jsp',
			data:{flag:"checkValue",field:obj,value:inputValue} ,
			success:function(msg){
				if(jQuery.trim(msg)=='ok'){
					jQuery("div[id^='"+obj+"']").hide() ;
					jQuery("#"+obj+"_dui").show() ;
				}
				if(jQuery.trim(msg)=='fail'){
					jQuery("div[id^='"+obj+"']").hide() ;
					jQuery("#"+obj+"_cuo").show() ;
					jQuery("#"+obj+"_cuo_info").html("用户名不存在").show() ;
				}
			}
		}) ;
	}
	function mySubmit(){
	var canBinding = true ;
	jQuery(".cuo").each(function(){
		if(jQuery(this).attr("style").indexOf('none')==-1){
		  canBinding = false ;
		  return ;
		}
	}) ;
	if(!canBinding || jQuery.trim(jQuery("#mem_no").val())=='' || jQuery.trim(jQuery("#passw").val())==''){
		jQuery.jBox.tip("请检查帐号信息") ;
	}else{
	   jQuery.ajax({
	   	  url:'/tools/ajax.jsp' ,
	   	  type:'post',
	   	  data:{flag:"canBinding",mem_no:jQuery("#mem_no").val(),passw:jQuery("#passw").val(),sina_id:jQuery("#sina_id").val(),qq_id:jQuery("#qq_id").val()} ,
	   	  success:function(msg){
	   	     if(jQuery.trim(msg)=='ok'){
	   	     	 jQuery.jBox.tip("绑定成功") ;
	   	     	 setTimeout(function(){
	   	     	 	window.location.href="/manage/memberhome.jsp" ;
	   	     	 },1000) ;
