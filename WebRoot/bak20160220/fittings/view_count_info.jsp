...
<%
	...
	StringBuffer query =new StringBuffer("select add_date,mem_name,province,city,telephone,email,company from fittings_collection_view_info where flag = '"+flag+"' and parent_no = '"+no+"' and mem_no is not null and mem_no != '' order by add_date desc");
	String proStatSql = "select top 15 province,count(*) as count from fittings_collection_view_info where flag = '"+flag+"' and parent_no = '"+no+"' and mem_no is not null and mem_no != '' group by province order by count desc";
	try{
...
<div>
  ...
  <div align="center">
  	<input type="radio" id="viewType1" name="viewType" value="" style="border:none" onclick="window.location.href='view_count_info.jsp?no=<%=no%>'" checked="checked"/>会员浏览量统计
	<input type="radio" id="viewType2" name="viewType" value="" style="border:none" onclick="window.location.href='view_count_info_other.jsp?no=<%=no%>'"/>非会员浏览量统计
  </div>
  <div class="loginlist_right1">
    <table width="100%" border="0" class="list">
	  ...
	  %>
      <tr>
        <td><%=Common.getFormatStr(rs.getString("company"))%></td>
        <td><%=Common.getFormatStr(rs.getString("mem_name"))%></td>
        <td><%=Common.getFormatStr(rs.getString("telephone"))%></td>
