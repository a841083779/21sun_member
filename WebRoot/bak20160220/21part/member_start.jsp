...
<%
	...
	if(key==null||"".equals(key)||key.indexOf("--")==-1){
		userno = Common.getFormatStr(Common.decryptionByDES(Common.getCookies(request, "cookieMemNo")));
		password = Common.getFormatStr(Common.decryptionByDES(Common.getCookies(request, "cookiePassw")));
	}else{	
...
<input type="hidden" id="password" name="password" value="<%=Common.encryptionByDES(password)%>"/>
  <div style="width:220px;border-bottom:1px dashed #ddd;height:25px;margin-bottom:10px;"><span style="font-size:14px;font-weight:bold;">请选择杰配网会员类型</span></div>
  <input type="radio" name="membertype" value="1" style="border: 0px solid #C0C0C0 " checked />
  <span style="font-size:12px;">成为顾客</span>
  <input name="membertype" type="radio" style="border: 0px solid #C0C0C0" value="1" />
  <span style="font-size:12px;">成为老板</span> <br /><br />
