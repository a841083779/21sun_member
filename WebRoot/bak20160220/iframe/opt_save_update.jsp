...
String mypy=Common.getFormatStr(request.getParameter("mypy"));
	String tip_success = Common.getFormatStr(request.getParameter("tip_success"));
...
int result = 0;
try{
   if(null==session.getAttribute("loginRand")||Common.getFormatStr(request.getParameter("rand")).equals(Common.getFormatStr((String)session.getAttribute("loginRand")))){
	...
	if(result>0){
		if(!"".equals(is_close)){
			%>
	<form action="<%=is_close %>" target="_top" method="get" name="topForm" id="topForm"><input type="hidden" name="t" value="<%=Common.getToday("yyyyMMddHHmmss") %>" /></form>
			<%
		}
%>
	<script>
	<%
		if(!"".equals(tip_success)){
			%>alert("<%=tip_success %>");setTimeout(function(){document.topForm.submit();},1);<%
		}else{
			%>alert("OK！操作成功！");<%
		}
	%>
