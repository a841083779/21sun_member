...
String [][]rolePurviews = DataManager.fetchFieldValue(pool,"member_purview_new","purview_num,purview_name","flag=1 order by purview_num");

String [][]purviews = DataManager.fetchFieldValue(pool,"member_role_purview_new","purview_num","role_num='"+roleNum+"' order by purview_num");
%>
...
<form id="theform" name="theform" method="post" action="role_purview_opt_action.jsp" >
...
<%for(int i=0;rolePurviews!=null && i<rolePurviews.length;i++){%>
		tree.N["<%if((rolePurviews[i][0]).length()==4){%>0<%}%><%if(rolePurviews[i][0].length()>=7){%><%=rolePurviews[i][0].substring(0,4)%><%}%>_<%=rolePurviews[i][0]%>"] = "ctrl:sel;checked:0;T:<%=rolePurviews[i][1]%>;"
<%}%>	
