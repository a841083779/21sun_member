...
try{//====不是删除时的操作=====
 ...
 {result =DataManager.dataInsUpt(request, pool3, mypy,2,7);
	 //====单个文件生成====
	if(myvalue.equals("0"))
      {tempInfo=DataManager.fetchFieldValue(pool3,Common.decryptionByDES(mypy),"max(id)", " catalog_no='" + zd_catalog_no + "'");
        if(tempInfo!=null&&tempInfo[0][0]!=null)
         {myvalue=Common.getFormatInt(tempInfo[0][0]);}
     } 
	rentToHtml.singleToHtml(pool3,request,zd_catalog_no,myvalue);
	//==================
 }
    //=====生成静态页====
	rentToHtml.indexList(request,pool3,zd_catalog_no,zd_category);
