...
if(name!=null&&!"".equals(name)){
	searchStr+=" and title like '%"+name+"%'";
}else{
	name="";
}

String tablename="heka_list";
searchStr+=" order by add_date desc";
//SQL查询
String query = "select * from "+tablename+" "+searchStr; 
...
<table width="97%"  border="0" cellpadding="0" cellspacing="0">
    <tr height="12">
      <td bgcolor="#C6CEE3"><strong class="font_big">　贺卡管理</strong></td>
    </tr>
    </table>
</div>
			...
			<div align="center">
                ...
                <tr>
                  <td width="12%" class="title_bar"><span class="title_bar">
                    <input name="add_b" type="button" class="form_button" id="add_b" value="增加" onClick="openWin('heka_add.jsp','add',500,400)"></span></td>
                  <td width="70%" align="center" class="title1">
                    标题 ：<input name="name" type="text" id="name" value="<%=name%>" size="10">
...
<div align="center">
              <table width="97%" border="0" cellpadding="4" cellspacing="1" class="table_border_bg" style="background-color: #717EA2" id="table2">
                <tr bgcolor="#e8f2ff">
                 ...
                 <td width="8%" nowrap class="title1" bgcolor="#e8f2ff"><strong class="list_border_bg">标题</strong></td>
                 <td width="8%" nowrap class="title1" bgcolor="#e8f2ff"><strong class="list_border_bg">是否发布</strong></td>
                  <td width="8%" nowrap class="title1" bgcolor="#e8f2ff"><strong class="list_border_bg">发布日期</strong></td>
...
%>
                <tr  <%=j%2==0?"bgcolor='#E8EAF0'":"bgcolor='#FFFFFF'"%>>
                  ...
                  <td height="22" nowrap ><%=Common.getFormatStr(rs.getString("title"))%></td>
                   <td height="22" nowrap ><%=Common.getFormatStr(rs.getString("show_id")).equals("1")?"是":"否"%></td>
                  <td height="22" nowrap ><%=Common.getFormatStr(rs.getString("add_date"))%></td>
                  <td align="center" nowrap >
                  <a href="#" onClick="openWin('heka_add.jsp?id=<%=Common.getFormatStr(rs.getString("id"))%>','modi',500,400)">修改</a>&nbsp;&nbsp;
                  <a href="#" onClick="javascript:deleteData('<%=rs.getString("id")%>','<%=tablename %>');">删除</a></td>
                </tr>
                <%}%>
                <tr class="table_border_cell_bg" colspan="22">
                  <td colspan="10" bgcolor="#FFFFFF" align=center>
<a href="#" onClick="pageSumit('0');">首页</a> <a href="#" onClick="pageSumit('<%=pagination.getPrev()%>');">上一页</a> <a href="#" onClick="pageSumit('<%=pagination.getNext()%>');" >下一页</a> <a  href="#" onClick="pageSumit('<%=pagination.getLast()%>');" >末页</a>
	...
	%>	
	 ...
	 <%=pagination.getCurrenPages()%>/<%=pagination.getTotalPages()%>页  总计<%=pagination.getTotal()%>条				  </td>
                </tr>
              </table></div>
</form>			  
</body>
