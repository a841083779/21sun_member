...
//=====页面属性====
String tablename="zhidao_ans";
String pageSubName="rans_opt.jsp";
//======================
...
String rtitle=Common.decryptionByDES(request.getParameter("rtitle"));

StringBuffer query =new StringBuffer("select * from "+tablename+" where 1=1 and parent="+myvalue);
//得到参数
String findTitle=Common.getFormatStr(request.getParameter("findTitle"));
if(!findTitle.equals("")){
	query.append(" and context like '%"+findTitle+"%'");
}
try{
    ...
    <tr>
      <td valign="top"><table width="100%"  border="0" cellpadding="0" cellspacing="0">
        ...
        <table width="100%"  border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td height="15" width="50%"><input type="button" name="b_add" value="增加" onclick="openWin('<%=pageSubName+"?parent="+myvalue%>','win',800,600)"/>
              </td>
              <td width="50%">《<%=title %>》的回复————<a href="other_list.jsp?rid=<%=rid %>&title=<%=rtitle %>">返回上一级</a></td>
          </tr>
        </table>
        <table width="98%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="3%" height="30" align="center" bgcolor="e8f2ff"><strong>ID</strong></td>
            <td width="46%" bgcolor="e8f2ff"><strong>内容缩略</strong></td>
            <td width="20%" align="center" bgcolor="e8f2ff"><div align="center"><span class="p92z"><strong>回复人</strong></span></div></td>
...
%>
          <tr  <%=(k%2)==1?"bgcolor='#F9F9F9'":""%>>
            ...
            <%
            	if(Common.getFormatStr(rs.getString("context"))!=null&&Common.getFormatStr(rs.getString("context")).length()>30){
            		%><td><a href="#" onclick="openWin('<%=pageSubName%>?myvalue=<%=rs.getString("id")%>&rid=<%=rid %>','win',800,600)"><%=Common.getFormatStr(rs.getString("context")).substring(0,30)+"..."%></a></td><%
            	}else{
            		%><td><a href="#" onclick="openWin('<%=pageSubName%>?myvalue=<%=rs.getString("id")%>&rid=<%=rid %>','win',800,600)"><%=Common.getFormatStr(rs.getString("context"))%></a></td><%
            	}
             %>
            ...
            <td align="center"><div align="center"><span class="p92j"><a href="javascript:otherDeleteData('opt_delete.jsp','<%=Common.encryptionByDES(rs.getString("id"))%>','<%=Common.encryptionByDES(tablename)%>');">删除</a> &nbsp;&nbsp; <a href="#" onclick="openWin('<%=pageSubName%>?myvalue=<%=rs.getString("id")%>&rid=<%=rid %>','win',800,600)">修改</a></span></div></td>
          </tr>
          <%
}
%>
