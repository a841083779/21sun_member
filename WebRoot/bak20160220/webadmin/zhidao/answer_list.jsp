...
String pageSubName="answer_opt.jsp";
//======================
        ...
        <table width="100%"  border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td height="15" width="50%"><input type="button" name="b_add" value="增加" onclick="openWin('<%=pageSubName+"?parent="+myvalue%>','win',800,600)"/>
              ...
              <td width="50%">《<%=title %>》的回复————<a href="post_list.jsp">返回上一级</a></td>
          </tr>
        </table>
          ...
          <tr  <%=(k%2)==1?"bgcolor='#F9F9F9'":""%>>
            ...
            <%
            	if(Common.getFormatStr(rs.getString("context"))!=null&&Common.getFormatStr(rs.getString("context")).length()>30){
            		%><td><a href="#" onclick="openWin('<%=pageSubName%>?myvalue=<%=rs.getString("id")%>','win',800,600)"><%=Common.getFormatStr(rs.getString("context")).substring(0,30)+"..."%></a></td><%
            	}else{
            		%><td><a href="#" onclick="openWin('<%=pageSubName%>?myvalue=<%=rs.getString("id")%>','win',800,600)"><%=Common.getFormatStr(rs.getString("context"))%></a></td><%
            	}
             %>
