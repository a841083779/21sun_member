...
function refresh(){
 ...
 function onloadAjax(){
  ...
  function submityn(){
  theform.zd_pubdate.value=document.getElementById("pub_date_temp").value;
  var zd_class_arr = document.theform.zd_class;
   if(pubCount>=<%=PUBNUMS%> && zd_class_arr[0].checked){
			...
			$("#zd_category").focus();
			return false;
	}else if($("#zd_subcategory").val()==""){
			alert("请选择下属类别！");
			$("#zd_subcategory").focus();
			return false;
...
{var selected = $("input[name='zd_class'][checked]").val();
 if(selected == null){alert("请选择租赁类型！");
  $("#zd_title").val(""); 
...
<body>
  ...
  <div class="loginlist_right2"><span class="mainyh">发布租赁信息</span></div>
  ...
  <table width="95%" border="0" align="center" class="tablezhuce">
      ...
      <td height="22" class="list_cell_bg">
	      <input type="radio" name="zd_class" value="1"> 出租&nbsp;&nbsp;<input type="radio" name="zd_class" value="0">
	      求租</td>
    </tr>
    <tr>
       <td align="right" nowrap class="list_left_title"><strong><font color="#FF0000">*</font>标　　题：</strong></td>	
       <td class="list_cell_bg"><input name="zd_title" type="text" id="zd_title" size="60" maxlength="40" class="required"  onKeyDown="javascript:checkradio();"></td>
    </tr>
    ...
    <tr>
	 ...
	 <tr>
      ...
      <td height="22" class="list_cell_bg">
	  		<input name="zd_price" type="text" id="zd_price" size="20" maxlength="20">
      </td>
	 </tr>
    <tr>
	...
	<tr>
		...
		<td class="right"><input type="button" id="submitId" name="Submit" value="发 布" class="tijiao" style="cursor:pointer"  onClick="submityn()"/>
		</td>
	</tr>   
	</table>
