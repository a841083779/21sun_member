...
<script type="text/javascript"
			src="/scripts/validate/jquery.metadata.js"></script>
<script type="text/javascript"
			src="/scripts/validate/jquery.validate.min.js"></script>
<script type="text/javascript" src="/scripts/validate/messages_cn.js"></script>
<link href="/scripts/select_city/search.css" rel="stylesheet"
			type="text/css" />
<link href="/scripts/select_city/style.css" rel="stylesheet"
			type="text/css" />
<script src="/scripts/select_city/options.js" type="text/javascript"></script>
<script type="text/javascript">var JQ = jQuery</script>
<script src="/scripts/select_city/location_array.js"
			type="text/javascript"></script>
<script type="text/javascript"
			src="/scripts/jBox/jquery.jBox-2.3.min.js"></script>
<script type="text/javascript"
			src="/scripts/jBox/i18n/jquery.jBox-zh-CN.js"></script>
<link type="text/css" rel="stylesheet"
			href="/scripts/jBox/Skins/Blue/jbox.css" />
    ...
    <form action="opt_save_update.jsp" method="post" name="theform" id="theform" submited="true">
      <table width="95%" border="0" align="center" cellpadding="0"
        ...
        <tr>
          ...
          <td><input name="zd_title" type="text" id="zd_title" size="75"
									class="ri" validate="{required:true,minlength:5,maxlength:100}"
									onblur="javascript:checkCanPublish(this);" /></td>
        </tr>
        <tr>
          <th nowrap="nowrap"> <font>*</font> 省市： </th>
          <td><div class="select_box w_138 required" id="province_city"
									name="province_city"> 
              <script>			
        ...
        <tr>
          ...
          <td class="list_cell_bg"><select id="zd_probrand" name="zd_probrand">
              <option value="<%=probrand%>" id="changebrand"> <%=chooseBrand%> </option>
            </select>
            <select
									style="position: absolute; left: -2000px; display: none;"
									id="firm_3" name="sid" disabled="disabled">
              <option selected="selected" value="" style="display: none;"> </option>
            </select>
            <select
									style="position: absolute; left: -2000px; display: none;"
									id="car_2" name="mid" disabled="disabled">
              <option selected="selected" value=""></option>
            </select>
            <input type="checkbox" id="otherBrand" name="otherBrand"
									onclick="choseBrand();" />
            <span>其它</span>
            <label style="color: #FF6600; display: none;" id="errorlabel"> 请您选择品牌 </label>
        ...
        <tr>
          ...
          <td height="22" class="list_cell_bg my_list_cell_bg"><input type="checkbox" id="product_flag" name="product_flag"
									value="101001" class="required" />
            挖掘机
            <input type="checkbox" id="product_flag" name="product_flag"
									value="101002" />
            装载机
            <input type="checkbox" id="product_flag" name="product_flag"
									value="101003" />
            ...
            <input type="checkbox" id="product_flag" name="product_flag"
									value="103" />
            混凝土机械
            <input type="hidden" id="zd_product_flag" name="zd_product_flag"
									value="," />
            <input type="checkbox" id="product_flag" name="product_flag"
									value="104" />
            养路机械
        ...
        <tr>
          ...
          <td height="22" class="list_cell_bg"><input type="radio" name="zd_part_flag" id="zd_part_flag"
									value="101001" class="required" />
            发动机件
            <input type="radio" name="zd_part_flag" id="zd_part_flag"
									value="101003" />
            液压件
            <input type="radio" name="zd_part_flag" id="zd_part_flag"
									value="101002" />
        ...
        <tr>
          ...
          <td height="22" class="list_cell_bg"><textarea name="zd_descr" cols="60" rows="6" id="zd_descr"
									class="rt required"
									validate="{required:true,minlength:30,maxlength:2000}"></textarea></td>
        </tr>
        <%
							if (null != addflag && "1".equals(addflag)) {
						%>
        <tr>
          <th nowrap="nowrap"> <font>*</font>上传图片： </th>
          <td><div> <img src="/images/nopic.gif" id="my_equi_img" width="143"
										height="137" style="float:left;" /> <span style="margin-left: 3px; position: relative; top: -3px; float:left;"></span><span
										id="equi_img" style="position: relative; top: 5px; left: 8px; float:left;"></span>
              <input name="zd_img1" id="zd_img1" type="text" validate="{required:true}" style="float:left; width:0px; border:none;" />
            </div></td>
        </tr>
        <%
							} else {
        ...
        <tr>
          <td nowrap="nowrap">&nbsp;</td>
          <td><input id="submitId" type="submit" name="Submit" value="发布信息"
									class="registBtn01" style="cursor: pointer" /></td>
        </tr>
        <!-- 发布日期 -->
        <input type="hidden" id="zd_pub_date" name="zd_pub_date"
							value="<%=Common.getToday("yyyy-MM-dd HH:mm:ss", 0)%>" size="20"
							maxlength="60" readonly="true" class="moren"
							style="vertical-align: middle; line-height: 11px; padding: 1px 1px; 0; font: 12px arial" />
        <!-- 默认二手产品 -->
        <input type="hidden" id="zd_is_new" name="zd_is_new" value="2" />
        <!-- 默认显示 -->
        <input type="hidden" id="zd_is_show" name="zd_is_show" value="1" />
        <!-- 交易目的 -->
        <input type="hidden" id="zd_business_flag" name="zd_business_flag"
							value="<%=businessFlag%>" />
        <!-- 出售目的 -->
        ...
        <input name="zd_company" type="hidden" id="zd_company"
							value="<%=Common.getFormatStr((String) adminInfo.get("comp_name"))%>" />
        <input name="zd_mem_no" type="hidden" id="zd_mem_no"
							value="<%=Common.getFormatStr((String) adminInfo.get("mem_no"))%>" />
        <input name="zd_mem_name" type="hidden" id="zd_mem_name"
							value="<%=Common.getFormatStr((String) adminInfo.get("mem_name"))%>" />
        <input name="pub_date_temp" type="hidden" id="pub_date_temp"
							value="<%=Common.getToday("yyyy-MM-dd HH:mm:ss", 0)%>" />
        <input name="zd_mem_flag" type="hidden" id="zd_mem_flag"
							value="<%=Common.getFormatStr((String) adminInfo.get("mem_flag"))%>" />
        <input name="zd_orderno" type="hidden" id="zd_orderno"
							value="<%=Common.getToday("yyyyMMddHHmmss", 0)%>" />
        <input name="zd_probrandname" type="hidden" id="zd_probrandname"
							value="" />
        <!--  取得选中的checkbox名称插入数据库-->
        <input type="hidden" name="probrandname" id="probrandname"
							value="" />
        <input type="hidden" name="zd_uuid" id="zd_uuid"
...
jQuery(function(){
    ...
    jQuery("#theform").validate({
        ...
        event: "blur",
        messages:{
                  "zd_title":{required:" 请您输入发布时要显示的标题",minlength:" 最小长度为5个字符",maxlength:" 最大长度为100个字符"} ,
                  "product_flag": {required:" 请您至少选择一个行业类别"},
                  "zd_descr":{required:" 请添加产品描述",minlength:" 最少30个字符",maxlengh:" 最多2000个字符"} ,
                  "zd_part_flag":{required:" 请选择您的配件类别"},
				  "rand":{required:" 请输入验证码"},
				  "zd_img1":{required:" 请上传产品图片"}
                } ,
		...
		submitHandler: function(form) {  //通过之后回自己的调验证函数
			if(checkyz() && jQuery("#canPublish").val()=='1')
			 {
			 // ---------------------------------------------可能会产生重复提交表单现象-----------------------------------------------------
   ...
   jQuery(".my_list_cell_bg").find("input").bind("click",function(){
     	if(jQuery(this).attr("checked")==="checked")
     	{
          jQuery("#zd_product_flag").attr("value", jQuery("#zd_product_flag").val()+jQuery(this).val()+",");
     	}
    });      
});    
</script> 
<script type="text/javascript">      
function checkyz(){
	if(jQuery("#cond_search_form_work_location_show").html()=="")
     {
        jQuery("#selectcity").removeClass("noselectcity").addClass("selectcity") ;
     }  
     if(jQuery("#faker-zd_probrand-head div").html().replace(/\s+/g,"")=="请选择品牌" && !document.getElementById("otherBrand").checked)
     {
        jQuery("#errorlabel").attr("style","color:#FF6600;height:24px; display:inline-block; white-space:nowrap; background:url(/images/tip_ico.png) 5px 4px no-repeat; padding-left:23px;") ;
		jQuery("#faker-zd_probrand").addClass("faker error");
		return false;
     }else{
		 if(jQuery("#faker-zd_probrand-head div").html().replace(/\s+/g,"")=="请选择品牌" && document.getElementById("otherBrand").checked){
			 //jQuery("#zd_probrand").val("0");
			 jQuery("#zd_probrandname").val("其它");
		 }
...
jQuery(function(){
	jQuery("#faker-zd_probrand-body").bind("mouseout",function(){
    if(jQuery("#cond_search_form_work_location_show").html()=="")
    {
    	jQuery("#errorlabel").attr("style","color:#FF6600;display:none;") ;  
		jQuery("#faker-zd_probrand").removeClass("error");    
...
jQuery("#search_form_work_sublocation_select_div").bind("click",function(){
	if(jQuery("#cond_search_form_work_location_show").html()=="")
  	{
  	}
}) ;
jQuery("#faker-zd_probrand-body").bind("click",function(){
	// 给品牌赋值
     setTimeout("if(jQuery('#faker-zd_probrand-head div').html()!='请选择品牌') { jQuery('#errorlabel').attr('style','margin-left:-485px;color:#FF6600;display:none;') ;  jQuery('#zd_probrandname').attr('value',jQuery('#faker-zd_probrand-head div').html()) ; }",10) ;
	 jQuery('#faker-zd_probrand').removeClass('error');
   }) ;
})
</script>
...
<script type="text/javascript"
	src="http://resource.21-sun.com/plugin/upload/jr_upload.js"></script>
<script type="text/javascript" src="/scripts/dropdown-sytle1.js"></script>
<script type="text/javascript">
