...
if(name=='zd_part_flag' && value=='106'){
...
var obj = parent.document.getElementById("zd_"+name) ;
obj[0].innerHTML = val ; 
}
...
val="'"+val+"' target='_blank' ";
	 parent.document.getElementById('txt_zd_'+name).innerHTML = "<font color='red'><a href="+val+" ><font color='red'>点击此处查看图片</font></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:void(0)' onclick=\"reupload('zd_"+name+"');\"><font color='green'>点击此处重新上传</font></a></font>";
	parent.document.getElementById('ifr_zd_'+name).style.display='none';
	parent.document.getElementById('txt_zd_'+name).style.display='block';
}
</script>
<%
PoolManager pool4 = new PoolManager(5);
  ...
  if(obj!=null){
	...
	if(objType=="radio"){	
	  ...
	  setCheckbox("zd_<%=strSets[i]%>",zhi);
	     ...
	     if(obj.name!='zd_company' && obj.name!='zd_mem_flag' && zhi != "" && obj.name!='zd_is_review'){
...
if(strSets[i].equals("img1")){
	...
	out.println("parent.document.getElementById('my_equi_img').src=parent.document.getElementById('zd_img1').value;");
}
	if(strSets[i].equals("province")){
		//out.println("parent.set_city(parent.document.getElementById('zd_province'),zhi,parent.document.theform.zd_city,'');");
	}
%>  
</script>
<%
