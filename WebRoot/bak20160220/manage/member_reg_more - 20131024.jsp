	  ...
	  if(!zd_mem_no.equals("")){   
		...
		if(result>0){
			...
			if(zd_mem_no!=null && !zd_mem_no.equals("")){
				...
				String updatePartMemberSql = "update part_member set mem_name='"+zd_mem_name+"',sex="+sexInt+",mobile='"+zd_comp_phone+"',province='"+zd_per_province+"',city='"+zd_per_city+"',address='"+zd_comp_address+"',email='"+zd_comp_email+"',company_name='"+zd_comp_name+"',tel='"+zd_comp_mobile_phone+"',qq='"+zd_comp_qq+"',fax='"+zd_comp_fax+"',logo='"+zd_comp_logo+"',intro='"+zd_comp_intro+"' where mem_no='"+zd_mem_no+"'";
				//System.out.println("update part_member set mem_name='"+zd_mem_name+"',sex="+sexInt+",mobile='"+zd_comp_phone+"',province='"+zd_per_province+"',city='"+zd_per_city+"',address='"+zd_comp_address+"',email='"+zd_comp_email+"',company_name='"+zd_comp_name+"',tel='"+zd_comp_mobile_phone+"',qq='"+zd_comp_qq+"',fax='"+zd_comp_fax+"',logo='"+zd_comp_logo+"',intro='"+zd_comp_intro+"' where mem_no='"+zd_mem_no+"'";);
				DataManager.dataOperation(conn_part,updatePartMemberSql);
			}
			//修改配件网用户信息-end
		   if(!addflag.equals("0")){
		 	 out.print("<script>alert('完善信息成功!');window.location.href='/manage/member_reg_more.jsp';</script>");
		  }else{
			 out.print("<script>alert('完善信息成功!');window.location.href='/manage/member_reg_more.jsp';</script>");
		  }
		}else{
    ...
    <form method="post" name="theform" id="theform" onsubmit="return submityz2();">
      <table width="950" border="0" cellspacing="0" cellpadding="0" class="registMore">
        ...
        <tr>
          <th><font>*</font>手机号：</th>
          <td>
          	<input type="text" name="zd_comp_phone" id="zd_comp_phone" onblur="formyanzheng('zd_comp_phone')" class="ri" value="<%=comp_phone_session.equals("")?per_phone_session:comp_phone_session%>" onfocus="showText('zd_comp_phone')"/>
        ...
        <tr>
          ...
          <td>
          	<input name="zd_comp_mobile_phone" type="text" class="ri" id="zd_comp_mobile_phone" onblur="formyanzheng('zd_comp_mobile_phone')" value="<%=comp_mobile_phone_session.equals("")?perMobilePhoneSession:comp_mobile_phone_session%>"  onfocus="showText('zd_comp_mobile_phone')"/>
        ...
        <tr>
          <th>QQ：</th>
          <td><input type="text" name="zd_comp_qq" id="zd_comp_qq" class="ri" value="<%=comp_qq_session.equals("")?perQQSession:comp_qq_session%>" /></td>
        </tr>
        <tr>
          <th>传真：</th>
...
function submityz2(){
	...
	var info = (mem_name+comp_name+comp_intro).replace(/\s+/g,"") ;	
	var cango = '' ;
	jQuery.ajax({
		url:'/tools/ajax.jsp' ,
		type:'POST',
