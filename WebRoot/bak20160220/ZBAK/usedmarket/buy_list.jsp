...
<body>
  <div class="loginlist_right">
    <div class="loginlist_right2"><span class="mainyh">管理二手求购信息</span> </div>
    <div class="loginlist_right1">
    <form action="" method="get">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td>	
							<span style="font-size: 14px; font-weight: bold;">求购意向：</span><input type="text" name="title" style="height: 20px; width: 200px;" value="<%=title %>" />
      ...
      <table width="100%" border="0" class="list" style="margin-top: 5px;">
        <tr>		  
		  ...
		  <th width="42%"><div align="center">求购意向<span style="color:blue;">（点击查看）</span></div></th> 
        ...
        <tr>
			...
			<td><div style="text-align: left;"><a href="buy_opt.jsp?uuid=<%=Common.getFormatStr(rs.getString("uuid")) %>"><%=Common.getFormatStr(rs.getString("title")) %></a></div></td>
			...
			<td style="text-align: center;">
				<a href="buy_opt.jsp?uuid=<%=Common.getFormatStr(rs.getString("uuid")) %>">修改</a>
...
function seeMessage(infoid){
	var url = "message_list.jsp?infoid="+infoid+"&flag=4";
...
function doOneUpdate(uuid){
	jQuery.post("tools/update_pub_date.jsp",{'flag':'buy_one','uuid':uuid},function(data){
...
function doUpdate(){
	...
	jQuery.post("tools/update_pub_date.jsp",{'flag':'buy','uuid':uuidStr},function(data){
		if(jQuery.trim(data)=='ok'){
			alert("更新成功！");
			jQuery(".checkdel").removeAttr("checked");
		}else{
