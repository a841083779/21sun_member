...
//====查找db2中最大的id===
/*String tempdbId[][]=DataManager.fetchFieldValue(pool1,"sell", "max(convert(bigint,db2id))", "");
String db2Id="99999999";
if(tempdbId!=null)
db2Id=Common.getFormatStr(tempdbId[0][0]);
*/
				
ResultSet rs=DataManager.executeQueryDB2(conn2, "select MID,PUBDATE,EXTRADATE,ISPUBLISHED,CLICKED,TITLE,MODEL,BRAND,CATEGORY,SUBCATEGORY,PROVINCE,CITY,PLACE,PRICE,AMOUNT ,cast(DETAIL as varchar(4000)) as DETAIL,IMG1,IMG2,infoCLASS,id from USEDMARKET where  INFOCLASS=1 order by id");
//===mid='SHRIZHAO' and ISPUBLISHED=1
String catalog_no="";
String sql="";
String temp_img="";
int k=0;
...
sql="insert into sell_test(mem_no,PUBDATE,extradate,is_pub,CLICKED,TITLE,CATEGORY,SUBCATEGORY,PROVINCE,CITY,content,db2id,img1)values('"+Common.getFormatStr(rs.getString("MID"))+"','"+Common.getFormatDate("yyyy-MM-dd",rs.getDate("PUBDATE"))+"','"+Common.getFormatDate("yyyy-MM-dd",rs.getDate("EXTRADATE"))+"','"+Common.getFormatStr(rs.getString("ISPUBLISHED"))+"','"+Common.getFormatStr(rs.getString("CLICKED"))+"','"+Common.getFormatStr(rs.getString("TITLE"))+"','"+Common.getFormatStr(rs.getString("CATEGORY"))+"','"+Common.getFormatStr(rs.getString("SUBCATEGORY"))+"','"+Common.getFormatStr(rs.getString("PROVINCE"))+"','"+Common.getFormatStr(rs.getString("CITY"))+"','"+Common.getFormatStr(rs.getString("DETAIL")).replace("\r\n","<br>")+"','"+Common.getFormatStr(rs.getString("id"))+"','"+Common.getFormatStr(rs.getString("IMG1"))+"')";
...
{;}
finally{pool1.freeConnection(conn1);
