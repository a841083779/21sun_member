...
ResultSet rs=DataManager.executeQueryDB2(conn2, "select count(MID) as cou from WEBPARTS ");
out.println("---配件仓库-(181213)-");
	if(rs.next())
	{
	out.println(rs.getString("cou"));
	}
	out.println("--供应-(492063)--");
 rs=DataManager.executeQueryDB2(conn2, "select count(MID) as cou from part_market where subdirection=101");
	if(rs.next())
	{
	out.println(rs.getString("cou"));
	}	
