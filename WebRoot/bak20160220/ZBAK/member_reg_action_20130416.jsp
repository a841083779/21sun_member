...
<%@page import="com.sun.mail.iap.Response"%><%@ include
				...
				<%
					if (isReg == 1) { // 邦定帐号不发送邮件
						...
						if(referer!=null && !referer.equals("") ){
							if(referer.indexOf("openplatform/member_reg.jsp") == -1){
							try {
								WEBEmail.sendMailByUrl(perEmail, null, null, "商贸网注册激活邮件AD", "http://member.21-sun.com/tools/data/active_to_email.jsp?no=" + memNo + "&pa="
										+ Common.getFormatStr(request.getParameter("passw")) + "&em=" + perEmail, "utf-8");
							} catch (Exception e) {
								e.printStackTrace();
							}
							}
