...
<%
	...
	if(referer!=null && !referer.equals("") &&!reg_ips.equals("119.137.105.158")){
		if ((isRandOK && strIsNotNull)|| referer.indexOf("openplatform/member_reg.jsp") != -1) {
			//组装INSERT语句
			String insSql = "insert into member_info(add_ip,add_date,mem_no,mem_name,passw,per_phone,per_email,per_province,per_city,regi_ip,regi_city,regi_date,state,mem_flag,mem_flag_name,login_count,flag_job,flag_bbs,flag_blog,flag_space,flag_21part,reg_source,mem_flag_enddate,purpose,password_bak) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			String aa="insert into member_info(add_ip,add_date,mem_no,mem_name,passw,per_phone,per_email,per_province,"+"per_city,regi_ip,regi_city,regi_date,state,mem_flag,mem_flag_name,login_count,flag_bbs,flag_blog,flag_space,flag_21part)"+"values('"+addIp+"','"+addDate+"','"+memNo+"','"+memName+"','"+passw+"','"+perPhone+"','"+perEmail+"','"+perProvince+"','"+perCity+"','"+Common.getRemoteAddr(request,1)+"','"+perCity+"','"+Common.getToday("yyyy-MM-dd HH:mm:ss", 0)+"',1,'-1','普通会员','1','0','0','0','0')";
