...
try{
   ...
   if(flagvalue.equals("1")){
	...
	String info=zd_mem_name+zd_comp_name+zd_comp_intro;
	
	int filterkeywords=0;
	if(mem_flag.equals("1003")||mem_no_session.equals("hwasung")){
	//验证A类会员词汇
    ...
    if(filterkeywords_a!=null){
				...
				filterkeywords_a.put("麻将", "1");
				Iterator it = filterkeywords_a.keySet().iterator();  
                while (it.hasNext()){  
                String key;  
                key=(String)it.next();
				    if(info.indexOf(key)>=0||info.indexOf("票")>=0){
				      filterkeywords=1;
					  break ;
				    }
				}
	...
	if(mem_no_session.equals("zt11-2sgzx")){filterkeywords=0;}
	
	  if(!zd_mem_no.equals("")&&filterkeywords==0){   
		...
		if(result>0){
		   ...
		   if(!addflag.equals("0")){
			...
			out.print("<script>window.parent.location.href='/manage/memberhome.jsp?addflag="+addflag+"';</script>");
	   			 ...
	   			 out.print("<script>alert('请您不要发布国家禁止的敏感词汇或与工程机械行业无关的信息，请重新填写!');window.location.href='/manage/member_reg_more.jsp';</script>");
	   }
     }	
%>
...
function submityz2(){
	...
	var cango = '' ;
	 <%if(!mem_no_session.equals("zt11-2sgzx")){%>
	jQuery.ajax({
		url:'/tools/ajax.jsp' ,
		type:'POST',
		async:false,
