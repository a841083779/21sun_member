...
public class AuthImgVerify extends HttpServlet {
	/**
	 * 
	 */
	...
	private static char[] captchars = new char[] { '2', '3', '4', '5', '6', '7', '8', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'j', 'k',
			'm', 'n', 'p', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'M', 'N', 'P', 'Q',
			'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y' };// 0，1，2易和字母的o，l,z易混淆，不生成,可以生成汉字
	// 设置备选汉字，剔除一些不雅的汉字
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 随机写字母或汉字
		String test = "";
		if ((random.nextInt(2)) == 0 || true) {// 加true，永久性获取字母
			for (int i = 0; i < num; i++) {
				// 1.随机得到num个字母或数字
		...
		req.getSession(true).setAttribute(AuthImgVerify.SIMPLE_CAPCHA_SESSION_KEY, test);// 将字符串写入session
		...
		textTl.draw(graphics, 60, 60);// 对字体加投影，第二个是左右相距，越大越远，第三个参数是上下两层相距距离，越大越近

		int w = bi.getWidth();
		int h = bi.getHeight();
		shear(graphics, w, h, c);// 使图片扭曲

		this.drawThickLine(graphics, 0, random.nextInt(ImageHeight) + 1, ImageWidth, random.nextInt(ImageHeight) + 1, 3, getRandColor(100,
				200));// 加一道线
