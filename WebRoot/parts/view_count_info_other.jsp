...
<%
	...
	String proStatSql = "select top 15 add_area,count(*) as count from parts_collection_view_info where flag = '"+viewFlag+"' and parent_no = '"+no+"' and (mem_no is null or mem_no = '') group by add_area order by count desc";
	String proStatAllSql = "select add_area,count(*) as count from parts_collection_view_info where flag = '"+viewFlag+"' and parent_no = '"+no+"' and (mem_no is null or mem_no = '') group by add_area order by count desc";
...
<div>
  ...
  <div align="center">
  	<input type="radio" id="viewType1" name="viewType" value="" style="border:none" onclick="window.location.href='view_count_info.jsp?no=<%=no%>&viewFlag=<%=viewFlag%>&siteFlag=<%=siteFlag%>'"/>会员<%=flagStr%>量统计
	<input type="radio" id="viewType2" name="viewType" value="" style="border:none" onclick="window.location.href='view_count_info_other.jsp?no=<%=no%>&viewFlag=<%=viewFlag%>&siteFlag=<%=siteFlag%>'" checked="checked"/>非会员<%=flagStr%>量统计
  </div>
...
%>
   xmlData=xmlData+"</chart>";
   var chart = new FusionCharts("/scripts/FusionCharts/charts/SSGrid.swf", "ChartId", "95%", "220","0","1");
   chart.setDataXML(xmlData);   
