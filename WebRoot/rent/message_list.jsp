...
<script>
  function viewMessage(objstr,objstr1,objstr2){		    
	self.location="../other/message_view.jsp?myvalue="+encodeURIComponent(objstr)+"&sort_flag="+encodeURIComponent(objstr1)+"&site_flag="+encodeURIComponent(objstr2);
  }
</script>
	<%
if(pool==null){
...
try{
	...
	if(session.getAttribute("memberInfo")!=null){   
		...
		mem_flag   = Common.getFormatStr(memberInfo.get("mem_flag"));  // 权限
	}
	StringBuffer query =new StringBuffer("select id,title,sender_mem_name,sender_mem_no,recipients_mem_no,add_date,is_read,sort_flag ,site_flag,info_id from "+tablename+" where sort_flag=1 and site_flag = 1");  //留言
	String searchStr="";
	if(!mem_no.equals("")&&!mem_flag.equals("1009")){
     ...
     <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0" class="list">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  class="list">
                <tr>
				  ...
				  <td width="12%" align="center" bgcolor="e8f2ff"><div align="center"><strong>查看</strong></div></td>
...
%>					
                <tr  <%=(k%2)==1?"bgcolor='#F9F9F9'":""%>>
				  ...
				  <a href="javascript:viewMessage('<%=Common.encryptionByDES(rs.getString("id"))%>','0','0')" >点击查看</a></div>
				  </td>
