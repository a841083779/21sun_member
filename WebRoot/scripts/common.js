...
//其它库的删除
function otherDeleteData(strurl,id,tablename){
	if(confirm("是否确定删除该信息，删除后将无法恢复!")){
	var url=strurl+"?mypy="+encodeURIComponent(tablename)+"&myvalue="+encodeURIComponent(id);
		$.ajax({
			   url: url,
			   type: 'POST',
			   dataType: 'html',
...
function setBackColor(id){
	for(var i=0;i<50;i++){
		if(document.all('m'+i)==null){continue;}
		if(i!=id){ 
			document.all('m'+i).className='ban01';
			
		}else{
			document.all('m'+i).className='show';
		}
	}
} 
...
//会员站上部导航颜色控制
function setTopNavStyle(id){
	
	for(var i=0;i<50;i++){
		if(document.all('topNav'+i)==null){continue;}
		if(i!=id){ 
			document.all('topNav'+i).className='';
			document.all('topNavTxt'+i).className='write';
		}else{
			document.all('topNav'+i).className='show';
			document.all('topNavTxt'+i).className='';
		}
	}
...
//各表单中上传iframe控制
function reupload(objname){
	document.getElementById('ifr_'+objname).style.display='block';
	document.getElementById('ifr2_'+objname).src=document.getElementById('ifr2_'+objname).src;
	document.getElementById('txt_'+objname).style.display='none';
}

//弹出层-页面
function openDialog(title,myurl,w,h)
{
 	// var dlg = new J.ui.dialog({title:title,width: mywidth,height:myheight,page:myurl,cover: true, rang: true, resize: false});
	var dg = new J.dialog({btnBar:false,cancelBtn:false,iconTitle:false,title:title,width: w,height:h,page:myurl,cover: true, rang: true, resize: true});
    dg.ShowDialog();
}
