...
<div class="C_layer registNew" style="width:100%;">
  ...
  <div class="C_content">
    <div class="registTable">
      <form action="/interface/login_and_regist/regist_action.jsp" method="post" name="theform" id="theform">
        <table width="98%" cellspacing="0" cellpadding="0" border="0">
          <tbody>
            <tr>
              <th width="18%"><font>*</font> 用户名：</th>
              <td width="82%"><input type="text" onkeyup="isSpecialChar('mem_no')" onblur="formyanzheng('mem_no')" onfocus="showText('mem_no')" class="ri forminupt" id="mem_no" name="mem_no" />
                <div style="display:none" id="mem_no_dui" class="diu"></div>
                <div style="display: block;" id="mem_no_cuo" class="cuo"></div>
                <div style="display: block;" id="mem_no_cuo_info" class="cuo1">请输入用户名称</div>
                <div style="display: none; clear: both;" id="mem_no_show_info">4-18个字母、数字、"-"或"_"，不支持中文和特殊符号。</div></td>
            </tr>
            <tr>
              <th><font>*</font> 密码：</th>
              <td colspan="2"><div style="clear:both">
                  <input type="password" onkeyup="pwStrength(this.value)" onblur="formyanzheng('passw')" onfocus="showText('passw')" class="ri forminupt" id="passw" name="passw" />
                  <div class="qr">
                    <ul>
                      <li class="" id="strength_L">弱</li>
                      <li class="" id="strength_M">中</li>
                      <li class="" id="strength_H">强</li>
                    </ul>
                  </div>
                  <div style="display:none" id="passw_dui" class="diu"></div>
                  <div style="display:none" id="passw_cuo" class="cuo"></div>
                  <div style="display:none" id="passw_cuo_info" class="cuo1"></div>
                </div>
                <div style="width:100%; clear:both;">
                  <div style="display:none; width:100%; float:left;" id="passw_show_info">6-16位字符，推荐使用字母、数字、"-"或"_"的组合</div>
                </div>
               </td>
            </tr>
            <tr>
              <th><font>*</font> 确认密码：</th>
              <td><input type="password" onblur="formyanzheng('passw2')" onfocus="this.className='regiHover'" class="ri forminupt" id="passw2" name="passw2" />
                <div style="display:none" id="passw2_dui" class="diu"></div>
                <div style="display:none" id="passw2_cuo" class="cuo"></div>
                <div style="display:none" id="passw2_cuo_info" class="cuo1"></div></td>
            </tr>
            <tr>
              <th><font>*</font> 邮箱：</th>
              <td colspan="2"><input type="text" onblur="formyanzheng('per_email')" onfocus="this.className='regiHover'" id="per_email" class="ri forminupt" name="per_email" />
                <div style="display:none" id="per_email_dui" class="diu"></div>
                <div style="display:none" id="per_email_cuo" class="cuo"></div>
                <div style="display:none" id="per_email_cuo_info" class="cuo1"></div></td>
            </tr>
            <tr>
              <th height="30"><font>*</font> 验证码：</th>
              <td colspan="2"><input type="text" onkeyup="formyanzheng('rand')" style="width:106px;" class="ri forminupt" id="rand" name="rand" />
                <div style="width:auto; float:left; padding-left:8px; padding-top:2px;">
                <img style="width:60px;height:20px;vertical-align:0px;" title="如果您看不清，请在图片上单击，可以更换验证码！" id="authImg" class="pass_reg_verifycode" src="/auth/authImgServlet" />
                <span style="vertical-align:3px;">看不清？</span>
                <a style="color:#005aa0; text-decoration:underline;vertical-align:3px;" onclick="refresh();" href="javascript:void(0);">换一张</a></div>
                <div style="display:none" id="rand_dui" class="diu"></div>
                <div style="display:none" id="rand_cuo" class="cuo"></div>
                <div style="display:none" id="rand_cuo_info" class="cuo1"></div></td>
            </tr>
            <tr>
              <th height="30">&nbsp;</th>
              <td><input type="button" class="registBtn" value="同意以下协议，提交" onclick="doRegist();" id="regid" name="regid" /></td>
            </tr>
          </tbody>
        </table>
        <div class="xieyi">
...
var forminupt = jQuery(".forminupt");
function refresh(){
	document.getElementById("authImg").src='/auth/authImgServlet?now=' + new Date();
}
forminupt.focus(function(){
	forminupt.css("border-bottom","#E0E0E0 1px solid").css("border-right","#E0E0E0 1px solid");
	forminupt.css("border-top","#848484 1px solid").css("border-left","#848484 1px solid");
	jQuery(this).css("border","1px solid blue");
});
forminupt.blur(function(){
	forminupt.css("border-bottom","#E0E0E0 1px solid").css("border-right","#E0E0E0 1px solid");
	forminupt.css("border-top","#848484 1px solid").css("border-left","#848484 1px solid");
});
...
function doRegist(){
	...
	jQuery("#theform").ajaxSubmit({
		type : "POST",
		success : function(rs){
			if(jQuery.trim(rs)=="true"){
				alert("注册成功，激活邮件已经发送到你的邮箱，请您激活后再进行登录，谢谢！");
				jQuery("#refParentBtn").click();
			}else{
				alert("系统出错，注册失败，请您稍后再试！");
			}
