...
<%@page import="com.jerehnet.util.common.CommonString"%><%@ include file ="/manage/config.jsp"%><%
	...
	query.append(" order by rec_index desc, pub_date desc");
	DBHelper dbHelper = DBHelper.getInstance() ;
	String rec_index_id = "" ;
	try{
	conn = pool.getConnection();  //SQL查询
	
	/*gaopeng add at 20131205 begin --查询三个月后供求数量*/
	//String[][] threeAgoCountSellandBuy = DataManager.fetchFieldOneValue(pool,
	//			"sell_buy_market_ago", "count(*)",
	//			" mem_no = '"+(String)adminInfo.get("mem_no")+"' and is_show=1 "); // 查询出会员3月前发出卖买的数量
	/*gaopeng add at 20131205 end --查询三个月后供求数量*/
	
	// 已推荐到首页的产品 id
	String rec_sql = "select id,title,pub_date,view_count,business_flag,probrand,probrandname,is_review,reviewContent  from "+tablename+" where mem_no = '"+(String)adminInfo.get("mem_no")+"' and rec_index=1" ;
...
<script language="javascript" type="text/javascript">
	function showMessage(){
		alert("尊敬的会员，为给广大客户提供一个专业稳定的信息平台，现只允许A类会员进行更新操作，如给您造成不便，敬请谅解！");
	}
...
<% if(!state2.equals("1")){ %>“批量更新”操作将使您的信息靠前显示，请及时更新。<%} %>
  ...
  <div class="TabTitleA">
    <ul id="myTab2">
      <li class="activeA" >三个月内</li>
      <li class="normalA" onclick="window.location.href='market_list_ago.jsp'">三个月前</li>
    </ul>
  ...
  <% if(!state2.equals("1")){ %> 
  	...
  	<%}else{ %>
  	<a href="javascript:showMessage();" style="width:80px;float:left" title="批量更新信息的发布时间，以便让信息在前面显示"><img src="../images/plgx.gif" border="0"/></a>
  	<%} %>
  <%} %>
    ...
    <table width="100%" border="0" class="list">
      ...
      <tr>
        ...
        <%
        	if(mem_flag.equals("1003")){
        		%>
        		<a href="javascript:;" rid='<%=rs.getString("id") %>' name="recToIndex" style="color:blue;">推荐到首页</a>
        		<%
        	}
        %>
        ...
        <% if(!state2.equals("1")){ %> 
        	...
        	<%}else{ %>
        	<a href="javascript:showMessage();">更新</a>
        	<%} %>
        <%} %>
                </div></td>
      </tr>
