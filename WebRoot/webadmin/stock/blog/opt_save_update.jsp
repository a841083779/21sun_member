...
String author=Common.getFormatStr(request.getParameter("zd_author"));
Connection conn = null;
try{
	conn = pool.getConnection();
	int result =DataManager.dataInsUpt(request, pool, mypy,2,0);
	DataManager.dataOperation(conn,"update stock_expert set lattest_pub=getdate() where name='"+author+"'");
	String[][] ids = DataManager.fetchFieldValue(pool,Common.decryptionByDES(mypy),"max(id)","");
if(result>0){
%>
<script type="text/javascript" src="http://stock.21-sun.com/tohtml/toBlogHtmlPage.jsp?id=<%=id.equals("0")?ids[0][0]:id%>"></script>
	<script>
