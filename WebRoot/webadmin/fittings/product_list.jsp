...
String tablename="fittings_products";
...
if(!title.equals("")){
	query.append(" and product_name like '%"+title+"%'");
}
String find_is_rec=Common.getFormatStr(request.getParameter("find_is_rec"));
if(find_is_rec.equals("2")){
	query.append(" and is_rec = 2 ");
}else if(find_is_rec.equals("1")){
	query.append(" and is_rec != 2 ");
}
...
if(!find_date_start.equals("")){
	query.append(" and CONVERT(varchar(12) ,update_date, 23 ) >='"+find_date_start+"' ");
}
String find_date_end=Common.getFormatStr(request.getParameter("find_date_end"));
if(!find_date_end.equals("")){
	query.append(" and CONVERT(varchar(12) ,update_date, 23 )<='"+find_date_end+"' ");
}
try{
      ...
      <td valign="top"><table width="100%"  border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="65%" align="center" nowrap="nowrap"> <span class="title1">会员编号：
			  ...
			  是否推荐：
			  <input name="find_is_rec" type="radio" id="find_is_rec0" value="" <%if(find_is_rec.equals("")){%> checked="checked"<%}%> style="border:0" />全部
              <input name="find_is_rec" type="radio" id="find_is_rec1" value="2" <%if(find_is_rec.equals("2")){%> checked="checked"<%}%> style="border:0" />是
			  <input name="find_is_rec" type="radio" id="find_is_rec2" value="1" <%if(find_is_rec.equals("1")){%> checked="checked"<%}%> style="border:0" />否
			  发布时间从
          ...
          <table width="100%"  border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td height="15"><input type="button" name="b_add" value="增加" onclick="openWin('product_opt.jsp','sell',650,600)"/>
				</td>
            </tr>
        ...
        <table width="98%" border="0" cellspacing="0" cellpadding="0">
            <tr>
			  ...
			  <td width="30%" bgcolor="e8f2ff"><strong>品名</strong></td>
            ...
            <tr  <%=(k%2)==1?"bgcolor='#F9F9F9'":""%>>
			  ...
			  <td title="点击预览"><a href="http://www.21peitao.com/product/detail_for_<%=Common.getFormatStr(rs.getString("id"))%>.htm" target="_blank"><%=Common.getFormatStr(rs.getString("product_name"))%><%=Common.getFormatStr(rs.getString("is_rec")).equals("2")?"(<font color='red'>推荐</font>)":""%></a></td>
              <td title="<%=Common.getFormatStr(rs.getString("id"))%>"><a href="#" onclick="openWin('product_opt.jsp?myvalue=<%=rs.getString("id")%>','sell',650,600)"><%=Common.getFormatStr(rs.getString("view_count")).equals("")?"0":Common.getFormatStr(rs.getString("view_count"))%></a></td>
              ...
              <td align="center"><div align="center"><span class="p92j"><a href="javascript:otherDeleteData('../fittings/opt_delete.jsp','<%=Common.encryptionByDES(rs.getString("id"))%>','<%=Common.encryptionByDES(tablename)%>');">删除</a> &nbsp;&nbsp; <a href="#" onclick="openWin('product_opt.jsp?myvalue=<%=rs.getString("id")%>','sell',650,600)">修改</a></span></div></td>
            </tr>
            <%
}
%>
